...
Thread::Thread(const char *threadName, bool callsJoin, int prior)
{
    name     = threadName;
    joinFlag = callsJoin;
    if (joinFlag)
        threadPort = new Port(name);
    priority = prior;
    realPriority = prior;
    stackTop = NULL;
    stack    = NULL;
...
Thread::Join()
{
    ...
    // Espera a que retorne el hijo.
    int msg;
    threadPort -> Receive (&msg);
}
...
void
Thread::ChangePriority(int prior)
{
    priority = prior;
}
...
OpenFileId
Thread::AddNewFile(OpenFile *ofile)
{
    for (unsigned i = 2; i < NUM_MAX_FILES; i++) 
    {
        if (ofilesids[i] == NULL)
            {
                ofilesids[i] = ofile;
...
void
Thread::CloseFile(OpenFileId ofileid)
{
    if (ofileid >= 2 && ofileid < NUM_MAX_FILES)
        ofilesids[ofileid] = NULL;
}

OpenFile *
Thread::GetFile(OpenFileId ofileid)
{
    if (ofileid >= 2 && ofileid < NUM_MAX_FILES)
        return ofilesids[ofileid];
    return NULL;
}
