...
template <class Item>
void
List<Item>::FindAndRemove(Item itemlookedfor)
{
    if (first != NULL && first -> item == itemlookedfor)
        first = first -> next;
    
    ListNode *newfirst;
    for (newfirst = first; newfirst != NULL && newfirst -> next != NULL; newfirst = newfirst -> next)
        if ((newfirst -> next) -> item == itemlookedfor)
